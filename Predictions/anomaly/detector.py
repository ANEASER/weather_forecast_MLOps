...
def save_json(column: str, value: float):
    existing_data = {}
    try:
        with open("anomaly/"+"weather_values.json", "r") as f:
    ...
    with open("anomaly/"+"weather_values.json", "w") as f:
...
def analyser(link: str,column: str, model_obj,threshold:float):
    ...
    if result == False:
        ...
        save_json(column,float(last_value)+round(float(values[-1]),1))
...
def run(link: str,column: str ,threshold: float = None, timer: datetime = None):
    ...
    #while True:
        #Load model
        model_path = "anomaly/"+column+'weather_anomaly_detector.h5'
        loaded_model = tf.keras.models.load_model(model_path)
        
        with open("anomaly/"+"threshold_values.json", "r") as f:
